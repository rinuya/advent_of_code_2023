#include "../main.hpp"

using namespace std;

void day9part1() {

    ifstream input("day9/input.txt");
    string line;

    int sum = 0;

    while (getline(input, line)) {
        int depth = 0;
        vector<string> rawReadings = splitString(line, ' ');
        vector<int> readings = vecStrToInt(rawReadings);
        map<int, vector<int>> differences;
        differences[0] = readings;
        while (!everyEntryEquals(differences[depth], 0)) {
            vector<int> currentDiff;
            for (int i = 0; i < differences[depth].size()-1; i++) {
                currentDiff.push_back(differences[depth][i+1]-differences[depth][i]);
            }
            depth++;
            differences[depth] = currentDiff;
        }
        depth--;
        while (depth != 0) {
            differences[depth].push_back(differences[depth][differences[depth].size()-1]+differences[depth+1][differences[depth+1].size()-1]);
            depth--;
        }
        sum += differences[depth][differences[depth].size()-1]+differences[depth+1][differences[depth+1].size()-1];
    }
    input.close();

    cout << "Day9  - Part1: The solution is " << sum << endl;
}